...
local outgoingEchoCache = {}
local outgoingEchoCacheQueue = {}
...
local function AddToEchoCache(translated, original)
    if not translated or not original then return end
    if not outgoingEchoCache[translated] then
        table.insert(outgoingEchoCacheQueue, translated)
    end
    outgoingEchoCache[translated] = original
    
    while table.getn(outgoingEchoCacheQueue) > 50 do
        local old = table.remove(outgoingEchoCacheQueue, 1)
        outgoingEchoCache[old] = nil
    end
end
...
local function ContainsLanguageChars(text, lang)
    ...
    if lang == "en" then
        -- Strip WoW color codes and hyperlink structures before checking
        -- to prevent "item", "quest", or hex codes from counting as English words
        ...
        for i = 1, string.len(plain) do
            ...
            elseif (b >= 65 and b <= 90) or (b >= 97 and b <= 122) then
                count = count + 1
                curWord = curWord + 1
                if curWord > maxWord then maxWord = curWord end
            else
                curWord = 0
            end
        end
        -- Threshold: at least one 4+ letter word OR 8+ total alpha characters
        return count >= 8 or maxWord >= 4
    end
...
local function PreprocessOutgoing(text)
    ...
    -- Group Finder LFXM (e.g. LF1M -> 4=1, LF 2 m -> 3=2)
    text = string.gsub(text, "[Ll][Ff]%s*([1-4])%s*[Mm]", function(x)
        return tostring(5 - tonumber(x)) .. "=" .. x
    end)
...
local function TriggerItemCache(itemId)
    ...
    local ok, err = pcall(itemCacheTooltip.SetHyperlink, itemCacheTooltip, itemString)
    if ok then
        DebugLog("Triggered cache for item:", itemId)
    else
        DebugLog("TriggerItemCache error for item", itemId, ":", err)
    end
end
...
-- Returns: linkType, linkData, displayText, colorCode, hasBrackets (or nils if parse fails)
local function ParseHyperlink(link)
    local colorCode = nil
    local linkType = nil
    local linkData = nil
    local displayText = nil
    local hasBrackets = true
    ...
    -- Find |h[ to find end of link data and start of display text (or just |h)
    local displayStart, displayStartEnd = string.find(link, "|h%[", hEnd)
    if not displayStart then
        displayStart, displayStartEnd = string.find(link, "|h", hEnd)
        hasBrackets = false
        if not displayStart then return nil end
    end

    -- Extract type:data between |H and |h[ (or |h)
    ...
    -- Find ]|h or |h to get display text
    local displayEnd
    if hasBrackets then
        displayEnd = string.find(link, "%]|h", displayStartEnd)
    else
        displayEnd = string.find(link, "|h", displayStartEnd)
    end
    ...
    return linkType, linkData, displayText, colorCode, hasBrackets
end

-- Extract ID from link data (format: id:...)
local function GetFirstIdFromLinkData(linkData)
...
local pfQuestLocaleKey = nil
local function GetPfQuestLocaleKey()
    if pfQuestLocaleKey then return pfQuestLocaleKey end
    if not pfDB or not pfDB["quests"] then return nil end
    local candidates = { "enUS-turtle", "turtle", "enUS" }
    for _, key in ipairs(candidates) do
        if pfDB["quests"][key] then
            pfQuestLocaleKey = key
            DebugLog("pfQuest locale key detected:", key)
            return key
        end
    end
    return nil
...
function WoWTranslate_GetEnglishQuestName(questId)
    local vanillaQuests = pfDB and pfDB["quests"] and pfDB["quests"]["enUS"]
    if not vanillaQuests then
        return nil  -- pfQuest not loaded
    end

    -- Try custom quests first (more specific)
    local localeKey = GetPfQuestLocaleKey()
    local customQuests = localeKey and pfDB["quests"][localeKey]
    if customQuests and customQuests[questId] then
        local entry = customQuests[questId]
        -- pfQuest-turtle marks removed vanilla quests as the string "_"
        if entry == "_" then
            -- Quest removed in Turtle WoW, fall through to vanilla lookup
        elseif type(entry) == "table" and entry["T"] then
            return entry["T"]
        end
    end
...
local function LocalizeHyperlink(link)
    ...
    local linkType, linkData, displayText, colorCode, hasBrackets = ParseHyperlink(link)
    ...
    if linkType == "item" then
        local itemId = GetFirstIdFromLinkData(linkData)
        DebugLog("  Item ID:", itemId)
        if itemId then
            -- GetItemInfo returns: name, link, quality, iLevel, ...
            local itemName, itemLink = GetItemInfo(itemId)
            
            -- pfDB fallback for uncached items (synchronous, no async wait needed)
            if not itemName and pfDB and pfDB["items"] and pfDB["items"]["enUS"] then
                local pfEntry = pfDB["items"]["enUS"][itemId]
                if pfEntry and type(pfEntry) == "table" and pfEntry["N"] then
                    itemName = pfEntry["N"]
                    DebugLog("  pfDB fallback successful for item:", itemId)
                end
            end
            ...
            if itemName then
                ...
                local result
                local formatStr = hasBrackets and "|h[%s]|h" or "|h%s|h"
                if colorCode then
                    result = "|c" .. colorCode .. "|H" .. linkType .. ":" .. linkData .. string.format(formatStr, itemName) .. "|r"
                else
                    result = "|H" .. linkType .. ":" .. linkData .. string.format(formatStr, itemName)
                end
    ...
    elseif linkType == "quest" then
        local questId = GetFirstIdFromLinkData(linkData)
        DebugLog("  Quest ID:", questId)
        if questId then
            local questName = WoWTranslate_GetEnglishQuestName(questId)
            ...
            if questName then
                ...
                local formatStr = hasBrackets and "|h[%s]|h" or "|h%s|h"
                if colorCode then
                    result = "|c" .. colorCode .. "|H" .. linkType .. ":" .. linkData .. string.format(formatStr, questName) .. "|r"
                else
                    result = "|H" .. linkType .. ":" .. linkData .. string.format(formatStr, questName)
                end
    ...
    -- Note: Spell localization not supported in vanilla WoW 1.12 (no GetSpellInfo API)
...
local function FindAllHyperlinks(text)
    ...
    while pos <= string.len(text) do
        ...
        -- Find the end of the hyperlink: |h[...]|h or |h...|h followed by optional |r
        local displayStart = string.find(text, "|h%[", linkStart)
        local hasBrackets = true
        if not displayStart then
            displayStart, displayStartEnd = string.find(text, "|h", linkStart)
            hasBrackets = false
        end
        ...
        if not displayStart then
            ...
            local displayEnd
            if hasBrackets then
                displayEnd = string.find(text, "%]|h", displayStartEnd)
            else
                displayEnd = string.find(text, "|h", displayStartEnd)
            end
            if not displayEnd then
                ...
                local linkEnd = displayEnd + (hasBrackets and 2 or 1)  -- Position after ]|h or |h
...
local function BuildTranslatableText(segments)
    ...
    for _, seg in ipairs(segments) do
        if seg.type == "text" then
            ...
            -- Space-pad the XML-like tag to prevent merging with CJK bytes.
            table.insert(parts, " [WT-LINK:" .. linkIndex .. "] ")
        end
    end
...
local function ReconstructMessage(segments, translatedText)
    ...
    for i = 1, linkCount do
        local placeholder = "%[WT%-LINK:" .. i .. "%]"
        -- Handle potential spaces introduced by translation e.g., [WT - LINK: 1]
        local placeholderFuzzy = "%[W%s*T%s*%-?%s*L%s*I%s*N%s*K%s*:%s*" .. i .. "%s*%]"
        
        DebugLog("Link", i, "content:", string.sub(linkContents[i] or "nil", 1, 80))

        local startPos, endPos = string.find(workText, placeholder)
        if not startPos then
            startPos, endPos = string.find(workText, placeholderFuzzy)
        end
...
local function MarkTranslatedDisplayName(rawName, displayName, unit)
    ...
    if class and RAID_CLASS_COLORS and RAID_CLASS_COLORS[class] then
        return RgbHex(RAID_CLASS_COLORS[class]) .. plain .. "|r|cFF888888(" .. rawName .. ")|r" .. TRANSLATED_NAME_MARK
    end
    return plain .. "|cFF888888(" .. rawName .. ")|r" .. TRANSLATED_NAME_MARK
end
...
local function BuildSenderPrefix(rawName, resolvedName, channel, guildDisplay)
    ...
    if channel then
        if isTranslated then
            ...
            coloredName = coloredName .. "|cFF888888(" .. rawName .. ")|r"
            local levelStr = ""
...
local function HookShaguPlatesNameplates()
    ...
    np.OnDataChanged = function(arg1, arg2)
        np.wtWoWTranslateBase(arg1, arg2)
        
        -- Dynamically resolve overlay to support both method (self, overlay)
        -- and function (overlay) call patterns without swallowing arguments
        local overlay = arg2
        if arg1 and arg1 ~= np then
            overlay = arg1
        end
...
local function HookChatFrames(force)
    ...
    for i = 1, NUM_CHAT_WINDOWS do
        ...
        if frame then
            local currentScript = frame:GetScript("OnEvent")

            -- Detect if another addon replaced our hook (e.g., WIM, Prat, or pfUI)
            if frame.WoWTranslate_Wrapper and currentScript ~= frame.WoWTranslate_Wrapper then
                frame.WoWTranslate_OrigScript = currentScript
                frame.WoWTranslateHooked = false
            end
            ...
            if not frame.WoWTranslateHooked then
                ...
                local origScript = frame.WoWTranslate_OrigScript or currentScript
                if not origScript then
                    ...
                    frame.WoWTranslate_Wrapper = function()
                        ...
                        local _ok, _err = pcall(function()
                            ...
                            if not messageShownInFrame then
                                -- Message was filtered by this frame (channel not shown, etc.)
                                -- Don't translate for this frame
                                FlushOriginal(); return
                            end
                            ...
                            local isEcho = false
                            if capturedArg2 == UnitName("player") and outgoingEchoCache[capturedArg1] then
                                isEcho = true
                            end
                            ...
                            local function TryPostToWIM(wtMsg)
                                if not wimWhisperUser then return false end
                                if lastWimMessageId == messageId then return true end
                                
                                local posted = false
                                if type(WIM_PostMessage) == "function" then
                                    WIM_PostMessage(wimWhisperUser, wtMsg, 3)
                                    posted = true
                                elseif type(WIM_Windows) == "table" and WIM_Windows[wimWhisperUser] then
                                    local wName = WIM_Windows[wimWhisperUser].GetName and WIM_Windows[wimWhisperUser]:GetName()
                                    if wName then
                                        local wimFrame = getglobal(wName .. "ScrollingMessageFrame")
                                        if wimFrame and wimFrame.AddMessage then
                                            wimFrame:AddMessage(wtMsg)
                                            posted = true
                                        end
                                    end
                                end
                                ...
                                if posted then
                                    lastWimMessageId = messageId
                                end
                                return posted
                            end

                            local function PostWTMsg(wtMsg)
                                local postedToWIM = TryPostToWIM(wtMsg)
                                if messageShownInFrame then
                                    capturedThis:AddMessage(wtMsg)
                                elseif not postedToWIM and wimWhisperUser then
                                    if lastWimMessageId ~= messageId then
                            ...
                            local sig = capturedEvent .. "|" .. (capturedArg1 or "") .. "|" .. (capturedArg2 or "") .. "|" .. (capturedArg4 or "")
                            if sig ~= lastMessageSignature then
                                messageCounter = messageCounter + 1
                                lastMessageSignature = sig
                                lastMessageId = tostring(GetTime()) .. "_" .. tostring(messageCounter)
                            end
                            local messageId = lastMessageId
                            ...
                            if not messageShownInFrame then
                                -- Chat addon compatibility: if a whisper is suppressed from standard
                                -- chat frames, we still want to translate it and route it to WIM or fallback.
                                if not wimWhisperUser then
                                    FlushOriginal(); return
                                end
                            else
                                if not frameTranslationTargets[messageId] then
                                    frameTranslationTargets[messageId] = {}
                                end
                                frameTranslationTargets[messageId][capturedThis] = true
                            end

                            if isEcho then
                                DebugLog("Echo cache hit")
                                local origMsg = outgoingEchoCache[capturedArg1]
                                frameTranslationTargets[messageId] = nil
                                ResolveNamesAndPost(origMsg, PostWTMsg)
                                return
                            end
                            ...
                            local apiQueued = WoWTranslate_API.Translate(textToTranslate, function(translation, err)
                                if translation and translation ~= "" then
                                    ...
                                    local targets = frameTranslationTargets[messageId]
                                    frameTranslationTargets[messageId] = nil
                                    ResolveNamesAndPost(reconstructed, function(wtMsg)
                                        local postedToWIM = TryPostToWIM(wtMsg)
                                        local postedToFrame = false
                                        if targets then
                                        ...
                                        if not postedToWIM and not postedToFrame and wimWhisperUser then
                                            if lastWimMessageId ~= messageId then
                                                DEFAULT_CHAT_FRAME:AddMessage(wtMsg)
                                                lastWimMessageId = messageId
                                            end
                            ...
                            if apiQueued and replacePendingData then
                                replacePendingKey = "r|" .. tostring(capturedThis) .. "|" .. messageId
                    ...
                    frame:SetScript("OnEvent", frame.WoWTranslate_Wrapper)
...
local MAX_PENDING_HARD_CAP = 100

local function CleanupPendingMessages()
    local now = GetTime()
    local count = 0
    for _ in pairs(pendingMessages) do count = count + 1 end
    
    -- Hard cap enforcement: remove oldest entries if over limit
    if count > MAX_PENDING_HARD_CAP then
        local toRemove = count - MAX_PENDING_HARD_CAP
        for msgId, pending in pairs(pendingMessages) do
            if toRemove > 0 then
                DebugLog("Force-removing old message:", msgId)
    ...
    local timeout = (count > 50) and 10 or 30
...
local function CleanupOutgoingQueue()
    local now = GetTime()
    local count = 0
    for _ in pairs(outgoingQueue) do count = count + 1 end
    
    if count > MAX_PENDING_HARD_CAP then
        local toRemove = count - MAX_PENDING_HARD_CAP
        for queueId, item in pairs(outgoingQueue) do
            if toRemove > 0 then
                DebugLog("Force-removing outgoing message:", queueId)
    ...
    for queueId, item in pairs(outgoingQueue) do
        if now - item.timestamp > timeout then
...
local function HookedSendChatMessage(msg, chatType, language, channel)
    ...
    WoWTranslate_API.TranslateOutgoing(textToTranslate, function(translation, err)
        ...
        if translation then
            ...
            -- Save to echo cache so incoming hook can show exact original text
            AddToEchoCache(reconstructed, queued.originalMsg)
...
-- Smart broadcast: sends message to the widest available audience.
-- If outgoing translation is enabled, it will be naturally caught by HookedSendChatMessage.
...
if GetLocale() == "zhCN" then
    ...
    L["  Incoming: "] = "  接收翻译: "
    L["  Outgoing: "] = "  发送翻译: "
    L["  Outgoing Hook: "] = "  发送挂钩: "
    L["  Glossary entries: "] = "  词汇表条目数: "
    L["  Cached translations: "] = "  已缓存翻译: "
    L["  Cache hit rate: "] = "  缓存命中率: "
    L["  Pending API requests: "] = "  等待中的API请求: "
    L["  Queued incoming: "] = "  队列中的接收: "
    L["  Queued outgoing: "] = "  队列中的发送: "
    L["  |cFFFF4444Last callback error:|r "] = "  |cFFFF4444最后的回调错误:|r "
    L["  |cFFFF4444API backoff active:|r "] = "  |cFFFF4444API回退生效中:|r "
    L["s remaining (use /wt reset to clear)"] = "秒剩余 (使用 /wt reset 清除)"
    L["Testing: "] = "测试："
    L["Cache hit: "] = "缓存命中："
    L["DLL not available"] = "DLL 不可用"
    L["Requesting from API..."] = "正在请求API..."
    L["API result: "] = "API 结果："
    L["API error: "] = "API 错误："
    L["Cache cleared"] = "缓存已清除"
    L["Debug mode: "] = "调试模式："
    L["Recent log entries:"] = "最近日志条目："
    L["Debug log cleared"] = "调试日志已清除"
    L["Outgoing translation enabled"] = "发送翻译已启用"
    L["Outgoing translation disabled"] = "发送翻译已禁用"
    L["Outgoing translation: "] = "发送翻译："
    L["Unknown channel: "] = "未知频道："
    L["  Valid channels: WHISPER, PARTY, GUILD, RAID, SAY, YELL, BATTLEGROUND, CHANNEL"] = "  有效频道: WHISPER, PARTY, GUILD, RAID, SAY, YELL, BATTLEGROUND, CHANNEL"
    L["[WoWTranslate] Outgoing channel settings:"] = "[WoWTranslate] 发送频道设置："
    L["  Usage: /wt outchannel <WHISPER|PARTY|GUILD|RAID|SAY|YELL|BATTLEGROUND|CHANNEL>"] = "  用法: /wt outchannel <WHISPER|PARTY|GUILD|RAID|SAY|YELL|BATTLEGROUND|CHANNEL>"
    L["[WoWTranslate] Outgoing prefix set to: "] = "[WoWTranslate] 发送前缀已设置为："
    L["[WoWTranslate] Current prefix: "] = "[WoWTranslate] 当前前缀："
    L["  Usage: /wt prefix <text>"] = "  用法: /wt prefix <文字>"
    L["[WoWTranslate] Usage: /wt broadcast <message> (or /wt b <message>)"] = "[WoWTranslate] 用法: /wt broadcast <消息> (或 /wt b <消息>)"
    L["  Translates and sends to Raid Warning, Raid, Party, or Say based on your group status."] = "  根据您的队伍状态翻译并发送到团队警告、团队、小队或说。"
    L["[WoWTranslate] Testing outgoing translation:"] = "[WoWTranslate] 测试发送翻译："
    L["  Input: "] = "  输入："
    L["Translation:"] = "翻译:"
    L["Error: "] = "错误："
    L["Commands:"] = "命令："
    L["  /wt show - Open configuration panel"] = "  /wt show - 打开配置面板"
    L["  /wt hide - Close configuration panel"] = "  /wt hide - 关闭配置面板"
    L["  /wt on|off - Enable/disable incoming translation"] = "  /wt on|off - 启用/禁用接收翻译"
    L["  /wt status - Show status"] = "  /wt status - 显示状态"
    L["  /wt reset - Recover if translations stop after alt-tab"] = "  /wt reset - 修复切屏后翻译停止的问题"
    L["  /wt clearcache - Clear cache"] = "  /wt clearcache - 清除缓存"
    L["  /wt debug - Toggle debug mode"] = "  /wt debug - 切换调试模式"
    L["  -- Outgoing --"] = "  -- 发送翻译 --"
    L["  /wt outgoing - toggle outgoing translation (on/off to set explicitly)"] = "  /wt outgoing - 切换发送翻译 (on/off 强制设置)"
    L["  /wt outchannel [type] - Show/toggle channel settings"] = "  /wt outchannel [type] - 显示/切换频道设置"
    L["  /wt prefix <text> - Set message prefix"] = "  /wt prefix <text> - 设置消息前缀"
    L["  /wt b <message> - Smart broadcast translated text (Raid/Party/Say)"] = "  /wt b <message> - 智能广播翻译文本 (团队/小队/说)"
    L["Reset OK — hooks reinstalled, DLL responding, cleared "] = "重置成功 — 挂钩已重新安装，DLL响应正常，已清除 "
    L[" stale request(s)"] = " 个挂起请求"
    L["Reset: hooks reinstalled but DLL not responding — try /reload"] = "重置: 挂钩已重新安装但DLL无响应 — 请尝试 /reload"
end
...
SlashCmdList["WOWTRANSLATE"] = function(msg)
    ...
    if cmd == "on" or cmd == "enable" then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("|cFF00FF00[WoWTranslate] " .. L["Enabled"] .. "|r")

    elseif cmd == "off" or cmd == "disable" then
        WoWTranslateDB.enabled = false
        DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[WoWTranslate] " .. L["Disabled"] .. "|r")

    elseif cmd == "status" then
        local dllStatus = WoWTranslate_API.IsAvailable()
            and "|cFF00FF00" .. L["Connected"] .. "|r"
            or "|cFFFF0000" .. L["Not loaded"] .. "|r"
        ...
        local outgoingStatus = WoWTranslateDB.outgoingEnabled
            and "|cFF00FF00" .. L["ON"] .. "|r"
            or "|cFFFF0000" .. L["OFF"] .. "|r"

        local hookStatus = IsOutgoingHookActive()
            and "|cFF00FF00" .. L["ACTIVE"] .. "|r"
            or "|cFFFF0000" .. L["INACTIVE"] .. "|r"

        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Status:"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  DLL: "] .. dllStatus)
        DEFAULT_CHAT_FRAME:AddMessage(L["  Incoming: "] .. (WoWTranslateDB.enabled and "|cFF00FF00" .. L["ON"] .. "|r" or "|cFFFF0000" .. L["OFF"] .. "|r"))
        DEFAULT_CHAT_FRAME:AddMessage(L["  Outgoing: "] .. outgoingStatus)
        DEFAULT_CHAT_FRAME:AddMessage(L["  Outgoing Hook: "] .. hookStatus)
        DEFAULT_CHAT_FRAME:AddMessage(L["  Glossary entries: "] .. glossaryCount)
        DEFAULT_CHAT_FRAME:AddMessage(L["  Cached translations: "] .. cacheStats.entries)
        DEFAULT_CHAT_FRAME:AddMessage(L["  Cache hit rate: "] .. string.format("%.1f%%", cacheStats.hitRate))
        DEFAULT_CHAT_FRAME:AddMessage(L["  Pending API requests: "] .. pendingCount)
        DEFAULT_CHAT_FRAME:AddMessage(L["  Queued incoming: "] .. queuedCount)
        DEFAULT_CHAT_FRAME:AddMessage(L["  Queued outgoing: "] .. outgoingQueuedCount)
        local cbErr = WoWTranslate_API.GetLastCallbackError and WoWTranslate_API.GetLastCallbackError()
        if cbErr then
            DEFAULT_CHAT_FRAME:AddMessage(L["  |cFFFF4444Last callback error:|r "] .. cbErr)
        end
        local rlActive, rlRemaining = WoWTranslate_API.GetRateLimitInfo()
        if rlActive then
            DEFAULT_CHAT_FRAME:AddMessage(L["  |cFFFF4444API backoff active:|r "] .. rlRemaining .. L["s remaining (use /wt reset to clear)"])
        end

    elseif cmd == "test" then
        local testText = arg or "\228\189\160\229\165\189"
        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Testing: "] .. testText)

        local cached, found = WoWTranslate_CacheGet(testText)
        if found then
            DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Cache hit: "] .. cached)
            return
        end

        if not WoWTranslate_API.IsAvailable() then
            DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[WoWTranslate] " .. L["DLL not available"] .. "|r")
            return
        end

        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Requesting from API..."])
        WoWTranslate_API.Translate(testText, function(result, err)
            if result then
                DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["API result: "] .. result)
                WoWTranslate_CacheSave(testText, result)
            else
                DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[WoWTranslate] " .. L["API error: "] .. (err or "unknown") .. "|r")
            end
        end)

    elseif cmd == "clearcache" then
        WoWTranslate_CacheClear()
        DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[WoWTranslate] " .. L["Cache cleared"] .. "|r")

    elseif cmd == "debug" then
        DEBUG_MODE = not DEBUG_MODE
        WoWTranslateDB.debugMode = DEBUG_MODE
        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Debug mode: "] .. (DEBUG_MODE and "|cFF00FF00" .. L["ON"] .. "|r" or "|cFFFF0000" .. L["OFF"] .. "|r"))

    elseif cmd == "log" then
        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Recent log entries:"])
    ...
    elseif cmd == "clearlog" then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Debug log cleared"])
    ...
    elseif cmd == "b" or cmd == "broadcast" then
        if arg and arg ~= "" then
            BroadcastMessage(arg)
        else
            DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] Usage: /wt broadcast <message> (or /wt b <message>)")
            DEFAULT_CHAT_FRAME:AddMessage("  Translates and sends to Raid Warning, Raid, Party, or Say based on your group status.")
        end
    ...
    elseif cmd == "reset" then
        ...
        if ok then
            DEFAULT_CHAT_FRAME:AddMessage("|cFF00FF00[WoWTranslate] " .. L["Reset OK — hooks reinstalled, DLL responding, cleared "] .. cleared .. L[" stale request(s)"] .. "|r")
        else
            DEFAULT_CHAT_FRAME:AddMessage("|cFFFF0000[WoWTranslate] " .. L["Reset: hooks reinstalled but DLL not responding — try /reload"] .. "|r")
        end
    ...
    elseif cmd == "hide" then
        ...
        DEFAULT_CHAT_FRAME:AddMessage("[WoWTranslate] " .. L["Commands:"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt show - Open configuration panel"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt hide - Close configuration panel"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt on|off - Enable/disable incoming translation"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt status - Show status"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt reset - Recover if translations stop after alt-tab"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt clearcache - Clear cache"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt debug - Toggle debug mode"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  -- Outgoing --"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt outgoing - toggle outgoing translation (on/off to set explicitly)"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt outchannel [type] - Show/toggle channel settings"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt prefix <text> - Set message prefix"])
        DEFAULT_CHAT_FRAME:AddMessage(L["  /wt b <message> - Smart broadcast translated text (Raid/Party/Say)"])
    end
end
...
local function InjectPfQuestZonesToGlossary()
    if not pfDB or not pfDB["zones"] or not pfDB["zones"]["enUS"] then return end
    if not pfDB["zones"]["zhCN"] then return end
    local addedIn = 0
    local addedOut = 0
    for id, zhName in pairs(pfDB["zones"]["zhCN"]) do
        local enName = pfDB["zones"]["enUS"][id]
        if type(zhName) == "string" and type(enName) == "string" then
            -- Inject into Incoming Glossary (zhCN -> enUS)
            if WoWTranslateGlossary and not WoWTranslateGlossary[zhName] then
                WoWTranslateGlossary[zhName] = enName
                addedIn = addedIn + 1
            end
            -- Inject into Outgoing Glossary (enUS -> zhCN)
            if WoWTranslateOutGlossary and not WoWTranslateOutGlossary[enName] then
                WoWTranslateOutGlossary[enName] = zhName
                addedOut = addedOut + 1
            end
        end
    end
    if addedIn > 0 or addedOut > 0 then DebugLog("Injected pfQuest zones:", addedIn, "incoming,", addedOut, "outgoing") end
end

local function InjectPfUILocalesToGlossary()
    if not pfUI_locale or not pfUI_locale["zhCN"] or not pfUI_locale["enUS"] then return end

    local addedIn = 0
    local addedOut = 0

    -- Use the spell icon textures as a bridge between EN and ZH spell names
    if pfUI_locale["zhCN"]["icons"] and pfUI_locale["enUS"]["icons"] then
        -- Create a reverse lookup for English icons: { "Spell_Frost_FrostBolt02" = "Frostbolt" }
        local enIconMap = {}
        for enName, iconPath in pairs(pfUI_locale["enUS"]["icons"]) do
            -- First match wins for shared icons (heuristic fallback)
            if not enIconMap[iconPath] then
                enIconMap[iconPath] = enName
            end
        end

        for zhName, iconPath in pairs(pfUI_locale["zhCN"]["icons"]) do
            local enName = enIconMap[iconPath]
            if enName then
                -- Inject into Incoming Glossary (zhCN -> enUS)
                if WoWTranslateGlossary and not WoWTranslateGlossary[zhName] then
                    WoWTranslateGlossary[zhName] = enName
    ...
    if addedIn > 0 or addedOut > 0 then DebugLog("Injected pfUI spells/icons:", addedIn, "incoming,", addedOut, "outgoing") end
end
...
local function OnAddonLoaded()
    ...
    if not pfDB then
        DEFAULT_CHAT_FRAME:AddMessage("|cFFFFFF00[WoWTranslate] Notice: pfQuest not detected. Quest link localization and pfDB fallbacks will be disabled.|r")
    else
        InjectPfQuestZonesToGlossary()
    end

    InjectPfUILocalesToGlossary()
    
    if WoWTranslate_InitOutgoingGlossary then
        WoWTranslate_InitOutgoingGlossary()
    end
...
local ITEM_CACHE_POLL_INTERVAL = 0.1   -- Poll every 100ms
